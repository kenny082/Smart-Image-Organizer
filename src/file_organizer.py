...
class FileOrganizer:
    ...
    def organize_images(self, dry_run: bool = True) -> Dict:
        ...
        for image_path in self.scan_images():
            try:
                ...
                location_info = self.geo_handler.get_location_info(coords) if coords else None
                
                # Generate new path
                new_path = self._generate_new_path(image_path, date_taken, location_info)
                ...
                self.operations_log.append({
                    ...
                    'location': self.geo_handler.format_location_string(location_info) if location_info else None,
                    'tags': tags
                })
    ...
    def _generate_new_path(self, image_path: Path, date_taken: Optional[str], 
                          location_info: Optional[Dict]) -> Path:
