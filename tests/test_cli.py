...
def test_organize_live_run(runner, source_dir, dest_dir):
    ...
    result = runner.invoke(app, ["organize", str(source_dir), str(dest_dir), "--no-dry-run"])
    assert result.exit_code == 0
...
def test_organize_with_ai_tagging(runner, source_dir, dest_dir):
    ...
    result = runner.invoke(app, ["organize", str(source_dir), str(dest_dir), "--use-ai"])
    assert result.exit_code == 0
...
def test_organize_with_log(runner, source_dir, dest_dir, tmp_path):
    ...
    result = runner.invoke(app, [
        "organize", str(source_dir), str(dest_dir), "--log", str(log_path)
    ])
    assert result.exit_code == 0
