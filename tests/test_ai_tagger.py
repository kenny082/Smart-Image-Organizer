...
def test_generate_tags(ai_tagger, sample_image, monkeypatch):
    ...
    test_categories = [
        ...
        "portrait",
        "wildlife",
    ]  # Only 3 categories for testing
    original_categories = ai_tagger.generate_tags.__defaults__[
        0
    ]  # Get the original confidence_threshold
    ai_tagger.generate_tags = (
        lambda image_path, confidence_threshold=original_categories: [
            cat
            for cat, prob in zip(test_categories, mock_probs[0])
